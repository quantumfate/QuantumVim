        ...
        options = {
            ...
            highlight = {
                ...
                disable = function(lang, buf)
                    ...
                    local status_ok, big_file_detected = pcall(
...
function M:config()
    ...
    local rainbow_ok, rainbow =
        pcall(require, "qvim.integrations.treesitter.rainbow")
    if rainbow_ok then
        rainbow:config()
    else
    ...
    qvim.integrations.treesitter.options =
        vim.tbl_deep_extend("keep", qvim.integrations.treesitter.options, {
            rainbow = qvim.integrations.treesitter.rainbow.options,
        })

    local climber_ok, climber =
        pcall(require, "qvim.integrations.treesitter.climber")
    if climber_ok then
        climber:config()
    else
...
function M:setup()
    ...
    if not status_ok then
        Log:warn(
            string.format("The plugin '%s' could not be loaded.", treesitter)
        )
        return
    end
    local repl_status_ok, repl_highlights =
        pcall(reload, "nvim-dap-repl-highlights")
    if repl_status_ok then
        repl_highlights.setup()
    else
        Log:warn(
            string.format(
                "The extension '%s' could not be loaded.",
                repl_highlights
            )
        )
    end
