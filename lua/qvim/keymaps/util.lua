...
function util.pairs_on_proxy(t)
    ...
    if mt then
        ...
        error "Proxy table doesn't implement a 'pairs' metamethod or doesn't have an underlying table specified in the 'index' metamethod that can be called by a 'pairs' function."
    end
...
function util.action_based_on_descriptor(
    descriptor,
    binding_call,
    binding_group_call
)
