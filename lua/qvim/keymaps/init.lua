...
local function parse_binding_to_descripted(
    descripted_keymaps,
    lhs,
    declaration,
    opts
...
local function parse_group_to_descripted(descripted_keymaps, declaration, bufnr)
    ...
    if descripted_keymaps[descriptor] then
        ...
        Log:warn(
            string.format(
                "An existing group '%s' will be overwritten.",
                descriptor
            )
        )
    end
...
local function fetch_bindings(bindings, descripted_keymaps, bufnr)
    if fn_t.length(bindings) > 0 then
        for lhs, declaration in pairs(bindings) do
            ...
            elseif
                type(lhs) == "number"
                and util.has_simple_group_structure(declaration)
            then
                -- group
                parse_group_to_descripted(
                    descripted_keymaps,
                    declaration,
                    bufnr
                )
