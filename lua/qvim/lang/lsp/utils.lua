...
function M.get_active_clients_by_ft(filetype)
	...
	for _, client in pairs(clients) do
		...
		if
			client.name ~= "null-ls"
			and vim.tbl_contains(supported_filetypes, filetype)
		then
...
function M.get_supported_filetypes(server_name)
	local status_ok, config = pcall(
		require,
		("lspconfig.server_configurations.%s"):format(server_name)
	)
...
function M.setup_document_highlight(client, bufnr)
	if
		qvim.plugins.vim_illuminate.enabled
		or qvim.plugins.vim_illuminate.enabled()
	then
		log.debug(
			"skipping setup for document_highlight, illuminate already active"
		)
		return
	end
