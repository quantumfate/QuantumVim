...
function loader.new(plugin_name, url)
  ...
  local plugin_spec = loader.load_lazy_config_spec_for_plugin(
    spec_require_path,
      ...
      for _, dep in pairs(plugin_spec.dependencies) do
        if type(dep) == "string" then
          ...
          if dep_ok and dep_name then
            ...
            log:debug(fmt("[core.loader] Failed to load spec for dependency '%s', of the plugin '%s'.", dep, plugin_name))
          end
        elseif type(dep) == "table" then
          dep_spec[#dep_spec+1] = dep
        else
          log:debug(fmt("[core.loader] Unknown dependency listed in '%s'. Should be type string or table but was '%s'.", plugin_name, type(dep)))
        end
      end
