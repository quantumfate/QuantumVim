	...
	keymaps:register(nil, {
		{
			...
			name = "+Python",
			bindings = {
				["m"] = { rhs = "<cmd>lua require('neotest').run.run()<cr>", desc = "Test Method" },
				["M"] = {
					rhs = "<cmd>lua require('neotest').run.run({strategy = 'dap'})<cr>",
					desc = "Test Method DAP",
				},
				["f"] = { rhs = "<cmd>lua require('neotest').run.run({vim.fn.expand('%')})<cr>", desc = "Test Class" },
				["F"] = {
					rhs = "<cmd>lua require('neotest').run.run({vim.fn.expand('%'), strategy = 'dap'})<cr>",
					desc = "Test Class DAP",
				},
				["S"] = { rhs = "<cmd>lua require('neotest').summary.toggle()<cr>", desc = "Test Summary" },
				c = { "<cmd>lua require('swenv.api').pick_venv()<cr>", desc = "Choose Env" },
			},
			options = {
				prefix = "<leader>",
			},
