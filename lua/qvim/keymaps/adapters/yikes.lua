...
local function on_group_activation(group)
    ...
    vim.cmd(
        "autocmd BufLeave,BufWinLeave <buffer> ++once lua remove_keymaps_for_group('"
            .. group
            .. "')"
    )
    vim.cmd(
        "autocmd InsertEnter <buffer> ++once lua remove_keymaps_for_group('"
            .. group
            .. "')"
    )
    ...
    if original_esc_keymap ~= nil then
        ...
        set_keymap(
            ...
            ":lua vim.schedule(function() remove_keymaps_for_group('"
                .. group
                .. "'); restore_original_esc_keymap() end)<CR><Esc>",
        ...
        _G.restore_original_esc_keymap = function()
            set_keymap(
                "n",
                original_esc_keymap.lhs,
                original_esc_keymap.rhs,
                original_esc_keymap.options
            )
        end
    else
        ...
        set_keymap(
            "n",
            "<Esc>",
            ":lua remove_keymaps_for_group('" .. group .. "')<CR><Esc>",
            { noremap = true, silent = true }
        )
    end
end
