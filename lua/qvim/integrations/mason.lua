...
function M.get_prefix()
	...
	return vim.tbl_get(qvim.integrations, "mason", "options", "install_root_dir") or default_prefix
end
...
---The mason setup function. The module will be required by
---this function and it will call the respective setup function.
---A on_config_done function will be called if the plugin implements it.
function M:setup()
	local status_ok, mason = pcall(reload, "mason")
	if not status_ok then
		Log:warn(string.format("The plugin '%s' could not be loaded.", mason))
		return
	end
