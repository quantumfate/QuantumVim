...
function Log:init()
  ...
  qvim_log.qvim.sinks[1].async = false -- HACK: Bug in structlog prevents setting async to false
  structlog.configure(qvim_log)
  local logger = structlog.get_logger "qvim"

  -- Overwrite `vim.notify` to use the logger
  if qvim.log.override_notify then
...
function Log:get_path()
  return string.format("%s/%s.log", get_cache_dir(), "qvim")
end
