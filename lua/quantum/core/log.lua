...
function Log:set_level(level)
  local logger_ok, logger = pcall(function()
    return require("structlog").get_logger "user"
  end)
...
function Log:init()
  ...
  user_log.user.sinks[1].async = false -- HACK: Bug in structlog prevents setting async to false
...
function Log:get_path()
  return string.format("%s/%s.log", get_cache_dir(), "user")
end
