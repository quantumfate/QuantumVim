...
declare -r QV_INSTALL_PREFIX="${INSTALL_PREFIX:-"$HOME/.local"}"
...
function main() {
  if [ ! -d "$QV_INSTALL_PREFIX" ]; then
    mkdir -p "$QV_INSTALL_PREFIX" || __invalid__prefix__handler
  fi
  download_neovim
  verify_neovim
  install_neovim
...
function install_neovim() {
  ...
  cp -r "$DOWNLOAD_DIR/$RELEASE_NAME/." "$QV_INSTALL_PREFIX"
  echo "Installation complete!"
  echo "Now you can run $QV_INSTALL_PREFIX/bin/nvim"
}

function __invalid__prefix__handler() {
  echo "Error! Invalid value for QV_INSTALL_PREFIX: [$INSTALL_PREFIX]"
