...
function M.setup(filetype)
    ...
    custom_spec_ok, custom_package = shared_util.register_custom_mason_package(filetype, "qvim.lang.dap.packages")
    ...
    elseif package_available then
        ...
        Log:error(fmt("No mason debug adapter package found for the filetype '%s'.", filetype))
        return
    end

    if package then
        shared_util.try_install_and_setup_mason_package(package, fmt("debug adapter on filetype %s", filetype),
            utils.setup_debug_adapter, { filetype })
    else
        Log:error(fmt(
