...
---@class loader
local loader = require("qvim.core.loader")
...
function core.init_plugin_configurations()
  ...
  log:debug(fmt("[core] Global qvim plugins table initialized. It total: '%s'", #qvim.plugins))
end
...
function core.load_lazy_spec()
  local lazy_spec = util.lazy_process_plugins(loader.new, false)
  log:debug(fmt("[core] Lazy spec initialized. Total plugins (without dependencies): %s", #lazy_spec))
  return lazy_spec
end
...
function core.load_lazy_spec_light()
  local lazy_spec_minimal = util.lazy_process_plugins(loader.new, true)
  log:debug(fmt("[core] Minimal lazy spec initialized. Total plugins (without dependencies): %s", #lazy_spec_minimal))
