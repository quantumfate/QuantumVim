...
local defaults = {
	insert_mode = {
		-- navigation
		["<A-n>"] = { rhs = "<Esc>:m .+1<CR>==gi", desc = "Move current line up" },
		["<A-t>"] = { rhs = "<Esc>:m .-2<CR>==gi", desc = "Move current line down" },
	...
	normal_mode = {
		...
		["<C-t>"] = { rhs = "<C-w>j", desc = "Go to lower window" },
		["<C-n>"] = { rhs = "<C-w>k", desc = "Go to upper window" },
		["<C-s>"] = { rhs = "<C-w>l", desc = "Go to right window" },
		-- Resize with arrows
		...
		["<A-n>"] = { rhs = ":m .+1<CR>==", desc = "Move current line up" },
		["<A-t>"] = { rhs = ":m .-2<CR>==", desc = "Move current line down" },
		...
		-- Navigate buffers
		["<C-j>"] = { rhs = ":bnext<CR>", desc = "Navigate to right buffer" },
		["<C-k>"] = { rhs = ":bnext<CR>", desc = "Navigate to left buffer" },
	},
	term_mode = {
		-- Terminal window navigation
		["<C-h>"] = { rhs = "<C-\\><C-N><C-w>h", desc = "Go to left terminal" },
		["<C-t>"] = { rhs = "<C-\\><C-N><C-w>j", desc = "Go to lower terminal" },
		["<C-n>"] = { rhs = "<C-\\><C-N><C-w>k", desc = "Go to upper terminal" },
		["<C-s>"] = { rhs = "<C-\\><C-N><C-w>l", desc = "Go to right terminal" },
	},
	visual_mode = {
		-- Better indenting
	...
	visual_block_mode = {
		...
		["<A-t>"] = { rhs = ":m '>+1<CR>gv-gv", desc = "Move current line down" },
		["<A-n>"] = { rhs = ":m '<-2<CR>gv-gv", desc = "Move current line up" },
	},
	command_mode = {
		-- navigate tab completion with <c-j> and <c-k>
		-- runs conditionally
		["<C-t>"] = {
			rhs = 'pumvisible() ? "\\<C-n>" : "\\<C-j>"',
			desc = "Navigate tab completion down",
			expr = true,
			noremap = true,
		},
		["<C-n>"] = {
			rhs = 'pumvisible() ? "\\<C-p>" : "\\<C-k>"',
			desc = "Navigate tab completion up",
			expr = true,
			noremap = true,
		},
	},
