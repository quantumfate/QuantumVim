...
function MethodService:init(method)
	...
	instance.method_str_arg = require("qvim.lang.null-ls._meta").method_to_string_arg()[method]
...
function MethodService:register_sources(configs)
	...
	for _, config in ipairs(configs) do
		...
		local type = self.method == null_ls.methods.CODE_ACTION and "code_actions"
			or null_ls.methods[self.method]:lower()
		local source = type and null_ls.builtins[type][name]
		self.log:debug(string.format("Received request to register [%s] as a %s source", name, type))
		if not source then
			self.log:error("Not a valid source: " .. name)
		elseif is_registered({ name = source.name or name, method = self.method }) then
			self.log:trace(string.format("Skipping registering [%s] more than once", name))
		else
			local command = MethodService.find_command(source._opts.command) or source._opts.command
...
function MethodService:setup(generic_configs)
	...
	if #registered > 0 then
		self.log:debug("Registered the following " .. self.method_str_arg .. ": " .. unpack(registered))
	end
end
