...
keymap.mt = {
    __index = function(t, lhs)
        if type(lhs) == "string" then
            ...
            error(
                string.format(
                    "Failed to index keymap. The left hand side of a binding must be a string but is '%s'",
                    type(lhs)
                )
            )
        end
    ...
    __newindex = function(t, lhs, other)
        if type(lhs) == "string" then
            ...
            if type(other) == "table" then
                ...
                error(
                    string.format(
                        "Error creating binding '%s'! A binding must have a table value but was '%s'.",
                        lhs,
                        type(other)
                    )
                )
            ...
            error(
                string.format(
                    "The left hand side of a binding must be a string but is a '%s'",
                    type(lhs)
                )
            )
        end
