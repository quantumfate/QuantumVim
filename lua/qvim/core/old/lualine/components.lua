...
if qvim.colorscheme == "lunar" then
    branch = "%#SLGitIcon#"
        .. qvim.icons.git.Branch
        .. "%*"
        .. "%#SLBranchName#"
end
...
return {
    ...
    lsp = {
        function()
            ...
            local formatters =
                require "qvim.lang.null-ls.methodservice.formatters"
            local supported_formatters = formatters:list_registered(buf_ft)
            vim.list_extend(buf_client_names, supported_formatters)

            local diagnostics =
                require "qvim.lang.null-ls.methodservice.diagnostics"
            local supported_diagnostics = diagnostics:list_registered(buf_ft)
            vim.list_extend(buf_client_names, supported_diagnostics)

            local code_actions =
                require "qvim.lang.null-ls.methodservice.code_actions"
            ...
            local unique_client_names =
                table.concat(make_unique(buf_client_names), ", ")
            local language_servers = string.format("[%s]", unique_client_names)

            if copilot_active then
                language_servers = language_servers
                    .. " "
                    .. qvim.icons.git.Octoface
                    .. " "
            end
