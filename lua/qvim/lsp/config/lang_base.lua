...
function M:lsp_set_server_settings(lsp_server)
  ...
  local status_ok, lsp_settings = utils:require_module("qvim.languages.lsp.settings.", true)
...
function M:hook_server_extension_config_with_function(server_opts)
	...
	-- inject server opts
	local status_ok, conf = pcall(require, "qvim.languages.lang." .. self.lsp_server)
	if not status_ok then
		return
	else
