...
local binding_group_constants =
    require("qvim.keymaps.constants").binding_group_constants
...
local  ---@class bindingbinding, ---@class groupgroup, ---@class keymapkeymap, ---@class defaultdefault, ---@class modemode, ---@class descriptor
    descriptor = nil, nil, nil, nil, nil, nil

--[[
    Require paths
]]
local  ---@class stringpath_binding, ---@class stringpath_group, ---@class stringpath_keymap, ---@class stringpath_default, ---@class stringpath_descriptor = "", "", "", "", ""
...
function util.normalize_keymap(lhs, bind)
    ...
    if
        bind[constants.neovim_options_constants.rhs]
        and bind[constants.neovim_options_constants.desc]
    then
        return bind
    end
    ...
    if type(bind) == "table" and #bind <= 2 then
        ...
        elseif bind[constants.neovim_options_constants.desc] then
            ...
            bind[constants.neovim_options_constants.rhs] =
                bind[constants.rhs_index]
            bind[constants.neovim_options_constants.desc] =
                bind[constants.desc_index]
...
util.process_group_memeber_mt = function(t, idx, other)
    ...
    if type(idx) == "number" or type(idx) == "string" then
        if type(other) == "table" then
            if
                other[binding_group_constants.key_binding_group]
                and type(other[binding_group_constants.key_binding_group]) == "string"
                and other[binding_group_constants.key_binding_group] ~= ""
            then
                ...
                Log:error(
                    string.format(
                        ...
                        getmetatable(t),
                        type(other[binding_group_constants.key_binding_group])
                    )
                )
            end
        else
            Log:debug(
                string.format(
                    "A group '%s' needs to be a table but was '%s'",
                    t,
                    type(other)
                )
            )
        ...
        Log:error(
            string.format(
                "A group's '%s' index must be a number or a string but was '%s'",
                t,
                type(idx)
            )
        )
...
util.set_binding_mt = function(_lhs, _binding, _options)
    ...
    local new_options =
        setmetatable(_options or {}, { __index = default.binding_opts })
